...
gis.map.mapPage.prototype.SVG = function (argument) {
	...
	var lineJsonDataArr = [];
	this.chinaJsonDataApi(this.pathData != '../js/geojson/china-province.geojson' ? '../'+this.pathData : this.pathData, function(back){
	...
	var opcity = 1;
	var checkLine = null;
	this.chinaJsonDataApi('js/nongshangwd.json', function(back){
		for (var i=0; i < back.length; i++) {
			var point=null;
			var mpoint = gis.map.convertLatLngToMPoint([back[i].经度, back[i].纬度]);
			...
			var pointPath = [
			  "M"+point[0]+" "+(point[1]+randoNum)+" "+"a"+randoNum+" "+randoNum+" 0 1 1,0.01 0 z",
			   ...
			   "fill: none; stroke: rgba(0, 255, 0, 0.9); stroke-width: 6px; cursor:pointer;",
			   i
		   	];
		   	var strLen = gis.map.strLength(back[i].机构名称) * 7 + 30;
			var bad = [
    ...
    .on('mousemove', function(d){
    	this.setAttribute('style', "fill: rgba(255, 255, 255, 0.6); stroke: rgba(255, 255, 255, 0.9); stroke-width: 0.5; stroke-linejoin:round;stroke-linejoin: round; cursor:pointer;");
    	//This.lonLat.push(this.getAttribute('LonLat'));
	})
	...
	This.g.selectAll('path .arc-label')
	    ...
	    .on('mousemove', function(d){
	    	var tip1Arr = document.querySelectorAll('.tip1');
		    var tip2Arr = document.querySelectorAll('.tip2');
		    var textArr = document.querySelectorAll('.text');
		    for (var i=0; i < tip1Arr.length; i++) {
				if (this.getAttribute('nid') == tip1Arr[i].getAttribute('tip')) {
					tip1Arr[i].setAttribute('style', "fill:rgba(255, 0, 0, 1); stroke:rgba(255, 0, 0, 1);stroke-width:5; display:block; cursor:pointer;");
				};
			};
			for (var i=0; i < tip2Arr.length; i++) {
				if (this.getAttribute('nid') == tip2Arr[i].getAttribute('tip')) {
					tip2Arr[i].setAttribute('style', "fill:rgba(0, 0, 0, 1); stroke:rgba(0, 0, 0, 1);stroke-width:5; display:block; cursor:pointer;");
				};
			};
			for (var i=0; i < textArr.length; i++) {
				if (this.getAttribute('nid') == textArr[i].getAttribute('tx')) {
					textArr[i].setAttribute('style', "font-size: 12pt; text-anchor: middle; fill: rgb(255, 255, 255); display:block; cursor:pointer;");
				};
			};
    	})
	    .on('mouseout', function(d){
		    ...
		    for (var i=0; i < tip1Arr.length; i++) {
				if (this.getAttribute('nid') == tip1Arr[i].getAttribute('tip')) {
					tip1Arr[i].setAttribute('style', "fill:rgba(255, 0, 0, 1); stroke:rgba(255, 0, 0, 1);stroke-width:5; display:none; cursor:pointer;");
				};
			};
			for (var i=0; i < tip2Arr.length; i++) {
				if (this.getAttribute('nid') == tip2Arr[i].getAttribute('tip')) {
					tip2Arr[i].setAttribute('style', "fill:rgba(0, 0, 0, 1); stroke:rgba(0, 0, 0, 1);stroke-width:5; display:none; cursor:pointer;");
				};
			};
			for (var i=0; i < textArr.length; i++) {
				if (this.getAttribute('nid') == textArr[i].getAttribute('tx')) {
					textArr[i].setAttribute('style', "font-size: 12pt; text-anchor: middle; fill: rgb(255, 255, 255); display:none; cursor:pointer;");
				};
			};
	    });
...
gis.map.mapPage.prototype.mouseInOut = function(mousewheelLock) {
	...
	function onmousewheelFun(ev, de) {
		...
		} else if (ev.wheelDelta == 120 && This.option.zoom < 20 && lock) {
			lock = false;
